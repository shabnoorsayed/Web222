...
function formatNames() {
  for (var i = 0; i < users.length; i++) {
    var fullName = users[i].name.first + ' ' + users[i].name.last;
...
userUtils.getProfiles = function(width, height) {
  ...
  var profiles = function() {
    return users.map(function(profiles) {
      var profile = new Object();

      profile.id = profiles.id;
      var fullName = profiles.name.first + ' ' + profiles.name.last;
      profile.email = fullName + ' <' + profiles.contact.email + '>';

      if (height === undefined) {
        profile.image =
          'https://faces.com/user/' + profiles.id + '?width=' + width + '&height=' + width;
      } else {
        profile.image =
          'https://faces.com/user/' + profiles.id + '?width=' + width + '&height=' + height;
      }
      return profile;
    });
  };

  profilesArray = profiles(users);
...
userUtils.updateAccessCount = function(amount, ids) {
  let idPassed = Array.isArray(ids) ? ids : [ids];
  var updateArray = [];
  idPassed.forEach(x => {
    var user = users.find(({ id }) => id === x);
    if (user !== undefined) {
      user.accessCount += amount;
      var selected = new Object();
      selected.id = user.id;
      selected.accessCount = user.accessCount;
      updateArray.push(selected);
    }
  });
  return updateArray;
};
...
userUtils.search.byName = function(name, fuzzy) {
  ...
  if (fuzzy === true) {
    ...
    user = users.filter(
      usr =>
        usr.name.first.toLowerCase().startsWith(name) ||
        usr.name.first.startsWith(name) ||
        usr.name.last.toLowerCase().startsWith(name) ||
        usr.name.last.startsWith(name)
    );
  } else {
    user = users.filter(usr => usr.name.first === name || usr.name.last === name);
  }
  return user;
};
...
userUtils.search.byCountry = function(...countries) {
  ...
  while (i < countries.length) {
    var user = users.filter(usr => usr.address.country === countries[i]);
...
userUtils.createWelcomeEmails = function() {
  var filteredUser = users.filter(usr => usr.accessCount === 0);
  var welcomeEmails = new Object();
  filteredUser.forEach(user => {
    var fullName = user.name.first + ' ' + user.name.last;
    var city = user.address.city;
    var country = user.address.country;

    var template = `Dear ${fullName}
Welcome to imaginary application!  We're so happy you joined, and wanted
to take a moment to say hello. Our other users from ${city}, ${country} will
be glad to have you join them.
